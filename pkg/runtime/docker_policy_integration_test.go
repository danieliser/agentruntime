...
// TestDockerRestrictedWorkspaceQualification proves the policy-v1 boundary
...
if env -u HTTP_PROXY -u HTTPS_PROXY -u http_proxy -u https_proxy -u ALL_PROXY -u all_proxy curl --noproxy '*' --connect-timeout 3 --max-time 5 -fsS https://api.openai.com/ >/dev/null 2>&1; then
  exit 41
fi
	...
	handle, err := runtime.Spawn(ctx, SpawnConfig{
		...
		Request: &apischema.SessionRequest{Agent: "codex", Context: "clean", AutoDiscover: false,
			ExecutionPolicy: &apischema.ExecutionPolicy{
				Version: "1.0", Workspace: "ephemeral", WorkspaceRetention: "terminal_receipt",
