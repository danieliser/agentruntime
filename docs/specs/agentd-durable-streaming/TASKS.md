...
| ACT-1003 | IN PROGRESS | Define and resolve a versioned caller execution policy without silent widening. | Unsupported workspace/filesystem/network/approval/tool/MCP/mount policy fails before admission; the canonical resolved policy and SHA-256 are immutable in the durable request manifest and returned on create/inspect. No migration is written unless a reviewed additive schema plan proves the manifest is insufficient. | L |
