...
| ID | Status | Task | Acceptance evidence | Size |
|---|---|---|---|---|
| CMP-601 | IN PROGRESS | Add a compatibility adapter for current clients during v1 rollout. | The `agentd attach` CLI consumes only the v1 sequence WebSocket and v1 controls. Production named chats now spawn Claude/Codex through a durable v1 generation and send running-chat follow-ups through the same durable control ledger; chat JSON remains under the unified data root. TUI, dashboards, Go client, and chat history still consume compatibility routes or byte cursors. | M |
...
| CMP-604 | IN PROGRESS | Update docs, examples, SDK, and capability response for the v1 stream contract. | README, architecture, and contributor guidance now describe native Claude/Codex, durable v1 sequences/controls/receipts, `~/.agentd`, Docker reconstruction, and the explicit compatibility boundary. Go SDK, dashboards/TUI examples, and capability response remain. | M |
