...
| ID | Status | Task | Acceptance evidence | Size |
|---|---|---|---|---|
| DKR-501 | IN PROGRESS | Persist and label session ID, job key, request hash, generation, container ID, image reference/digest, and sandbox-profile version. | v1 admission persists session/generation and Docker durable labels/recovery metadata; Docker now resolves the concrete container image ID after launch, validates its `sha256:` identity, and persists it with fresh and resumed generations. Explicit sandbox-profile labeling remains. | M |
