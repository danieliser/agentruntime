...
| DKR-504 | IN PROGRESS | Recover terminal state when container exits while AgentD is down. | Docker recovery includes stopped containers, retained logs, `docker wait`, and matching `.State` inspection. Proven OOM kills persist `crashed`, `SIGKILL`, exit 137, and runtime timestamps; cancellation remains distinct through durable control intent. Native deadlines now commit requested/dispatched timeout proof, win a single terminal boundary, persist `timed_out`, and retain the original generation deadline across restart. Reliable non-OOM signal proof remains. | M |
...
| ID | Status | Task | Acceptance evidence | Size |
|---|---|---|---|---|
| CMP-601 | IN PROGRESS | Add a compatibility adapter for current clients during v1 rollout. | `agentd attach` and `agentd dispatch` consume v1 sequence events; the Go client exposes durable dispatch/inspect/replay/raw-stream methods. Production named chats spawn Claude/Codex through a durable v1 generation and send follow-ups through the same control ledger. TUI, dashboards, legacy Go methods, and chat history still consume compatibility routes or byte cursors. | M |
...
| CMP-603 | IN PROGRESS | Remove sidecar WS normalization/replay after Gate G2. | The sidecar binary, local sidecar runtime, Docker health/port/WS path, WS runtime handle, sidecar image build, and non-native Grok/Cursor registration are removed. The unversioned daemon bridge and byte-cursor compatibility routes remain until CMP-601 migrates current clients. | M |
| CMP-604 | IN PROGRESS | Update docs, examples, SDK, and capability response for the v1 stream contract. | README, architecture, and contributor guidance describe native Claude/Codex, durable v1 sequences/controls/receipts, `~/.agentd`, Docker reconstruction, and the compatibility boundary. The Go client has typed durable session/event/capability methods and stable one-shot dispatch keys. `/api/v1/capabilities` reports AgentD/API/event-schema/provider/runtime/replay/reconstruction/plugin compatibility; dashboards/TUI examples remain. | M |
