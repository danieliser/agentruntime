...
| DKR-504 | IN PROGRESS | Recover terminal state when container exits while AgentD is down. | Docker recovery includes stopped containers, retained logs, `docker wait`, and matching `.State` inspection. Proven OOM kills persist `crashed`, `SIGKILL`, exit 137, and runtime timestamps; cancellation remains distinct through durable control intent. Reliable non-OOM signal proof and timeout qualification remain. | M |
