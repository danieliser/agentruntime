...
| ID | Status | Task | Acceptance evidence | Size |
|---|---|---|---|---|
| CMP-601 | IN PROGRESS | Add a compatibility adapter for current clients during v1 rollout. | `agentd attach`, `agentd dispatch`, the TUI, and the embedded dashboard consume v1 sequence events and durable HTTP controls. TUI and server chat history are reconstructed from immutable v1 ledgers. The Go client exposes only durable dispatch/list/inspect/replay/raw-stream plus prompt/steer/interrupt/cancel/terminate/resume/receipt methods. Production named chats spawn Claude/Codex through a durable v1 generation and send follow-ups through the same control ledger. Only unversioned server session routes still expose compatibility byte cursors; the redundant standalone dashboard and chat WebSocket proxy were retired. | M |
