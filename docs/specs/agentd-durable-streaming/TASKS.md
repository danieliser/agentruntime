...
| CMP-604 | IN PROGRESS | Update docs, examples, SDK, and capability response for the v1 stream contract. | README, architecture, and contributor guidance describe native Claude/Codex, durable v1 sequences/controls/receipts, `~/.agentd`, Docker reconstruction, and the compatibility boundary. The Go client now has typed durable session/event methods and stable one-shot dispatch keys; dashboards/TUI examples and capability response remain. | M |
