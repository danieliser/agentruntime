...
Status: **G0 + G1 + G2 + G3 + G4 + G5 + G6 APPROVED / PHASE 10 QUALIFIED / v2.1.2 RELEASED + ACTIVE**
...
| ACT-1011 | DONE | Qualify the installed daemon environment, not only the repository template. | The canonical launchd template includes both `/usr/local/bin` and `/opt/homebrew/bin`. The live `0600` plist exposes the same effective PATH under a `0700` data root with `0600` auth token; installed AgentD created a real Codex Docker generation after restart, cancelled it with a receipt, and returned HTTP 200 on repeated cancel. | S |
| ACT-1012 | DONE | Keep OpenTraces catch-up from blocking AgentD startup. | Initial best-effort observer replay is bounded to five seconds; a roughly 4,500-event real backlog reached HTTP health in six seconds, then advanced its exact checkpoint in the background while health remained 200. The full race suite and hosted Go 1.24.2/1.26.x CI pass. | S |
...
- [x] `Q-38` The installed launchd service's effective PATH resolves Docker and a real Docker create succeeds after service restart.
- [x] `Q-39` A slow or large OpenTraces catch-up cannot prevent the installed AgentD API from reaching healthy; checkpointed replay continues in the background.
...
- 2026-08-10 — ACT-1011/1012 and Q-38/39 completed in `v2.1.2` at `d0528ca431b30ab17dca943eaee140593459ff9c`; `v2.1.1` was published but superseded before activation. Six stamped wheels, trusted PyPI publication, and the GitHub release pass. The installed artifact verifies exactly, launchd reaches health in six seconds with the real OpenTraces backlog, the old `1718f7c8-...` orphan is permanently `indeterminate` with generation/event/receipt proof, and a no-model-turn Codex Docker create/cancel proves effective PATH plus idempotent control. OpenTraces checkpoint advanced from 1858 to 1862 while health remained HTTP 200; the stopped qualification container was removed and its temporary provider state moved to Trash.
