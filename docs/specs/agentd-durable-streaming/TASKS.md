...
| ID | Status | Task | Acceptance evidence | Size |
|---|---|---|---|---|
| CMP-601 | IN PROGRESS | Add a compatibility adapter for current clients during v1 rollout. | `agentd attach`, `agentd dispatch`, and the embedded dashboard consume v1 sequence events. The Go client exposes durable dispatch/list/inspect/replay/raw-stream methods. Production named chats spawn Claude/Codex through a durable v1 generation and send follow-ups through the same control ledger. TUI, the standalone dashboard, legacy Go methods, and chat history still consume compatibility routes or byte cursors. | M |
