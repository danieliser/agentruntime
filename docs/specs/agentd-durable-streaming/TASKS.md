...
| ID | Status | Task | Acceptance evidence | Size |
|---|---|---|---|---|
| CMP-601 | IN PROGRESS | Add a compatibility adapter for current clients during v1 rollout. | `agentd attach`, `agentd dispatch`, the TUI, and the embedded dashboard consume v1 sequence events and durable HTTP controls. TUI chat history is reconstructed from each session's immutable v1 ledger. The Go client exposes durable dispatch/list/inspect/replay/raw-stream methods. Production named chats spawn Claude/Codex through a durable v1 generation and send follow-ups through the same control ledger. Legacy Go methods and the server chat-history route still consume compatibility routes or byte cursors; the redundant standalone dashboard was retired. | M |
