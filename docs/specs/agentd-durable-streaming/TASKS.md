...
| ID | Status | Task | Acceptance evidence | Size |
|---|---|---|---|---|
| CMP-601 | IN PROGRESS | Add a compatibility adapter for current clients during v1 rollout. | The `agentd attach` CLI now consumes only the v1 sequence WebSocket and sends follow-up/steer/interrupt through idempotent v1 HTTP controls. TUI, dashboards, Go client, and chat history still consume compatibility routes or byte cursors. | M |
