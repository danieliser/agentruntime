...
Status: **G0 + G1 + G2 + G3 + G4 + G5 + G6 APPROVED / PHASE 10 QUALIFIED / v2.1.0 RELEASE IN PROGRESS**
...
| ACT-1009 | IN PROGRESS | Publish and activate the qualified `v2.1.0` installed artifact. | Tag/release CI builds six version-and-commit-stamped wheels; the installed binary passes `--require-build`, reports the qualified capability contract, starts against `~/.agentd`, and shows the local-only OpenTraces adapter healthy with zero lag. | M |
