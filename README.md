...
The execution sidecar and its port/health/WebSocket protocol are gone. Some unversioned daemon routes, the TUI, dashboards, Go client, and legacy chat-log pagination remain temporarily during v1 client migration. They use a separate compatibility cursor domain and must not be treated as v1 event sequences.
